...
(function () {
  ...
  class CanvasController {
    ...
    constructor(config) {
      ...
      // Cache min/max limits from the width input attributes.
      // The browser only clamps on blur/Enter, so JS must clamp too to
      // avoid huge mid-typed values locking up rendering.
    ...
    debouncedRender() {
      ...
      this.renderTimer = setTimeout(() => this.render(), 10);
    }
    ...
    /**
     * Read min/max limits from the width input element's attributes.
     *
     * @return {Object} {min, max} in mm.
     */
    readLimits() {
      const widthInput = document.getElementById('ipsr-width');
      const min = widthInput ? parseFloat(widthInput.getAttribute('min')) : NaN;
      const max = widthInput ? parseFloat(widthInput.getAttribute('max')) : NaN;
    ...
    /**
     * Clamp state width/height to cached [min, max] limits.
     *
     * The number input only clamps on blur/Enter, so a user can type a
     * very large value and trigger runaway canvas rendering. This pins
     * the values used for render without rewriting the input text.
     */
    normaliseParameters() {
      const { min, max } = this.limits;
      this.state.width = Math.min(Math.max(this.state.width, min), max);
      this.state.height = Math.min(Math.max(this.state.height, min), max);
    }
     ...
     */
    render() {
      // Clamp state to input min/max before any expensive work.
     ...
     */
    renderGrid() {
      const gridSize = this.config.grid_size * this.scale * 1.0;

      // Draw checkered pattern.
      this.ctx.fillStyle = '#d0d0d0';

      let colCount = Math.floor(this.canvasWidth / gridSize) + 2;
      let rowCount = Math.floor(this.canvasHeight / gridSize) + 2;

      // Wa always want an even numer of grid rows and columns.
      if (colCount % 2 !== 0) {
        colCount += 1;
      }
      ...
      // console.log(`${colCount} x ${rowCount}`);

      let centreX = this.canvasWidth * 0.5;
      let centreY = this.canvasHeight * 0.5;

      let row = 0 - Math.floor(rowCount * 0.5);
      let doesRowStartSolid = 1; //isCellSolid;
      while (row < rowCount * 0.5) {
        let isCellSolid = doesRowStartSolid;
        let col = 0 - Math.floor(colCount * 0.5);
        while (col < colCount * 0.5) {
          if (isCellSolid) {
            let x = centreX + col * gridSize;
            let y = centreY + row * gridSize;

            this.ctx.fillRect(x, y, gridSize, gridSize);
          }

          ++col;
          isCellSolid = 1 - isCellSolid;
        }

        doesRowStartSolid = 1 - doesRowStartSolid;
	...
	*/
    ...
    renderColorOverlay(outerSheet) {
      ...
      // Calculate opacity using linear scale.
      const maxDeviation = idealArea; // Maximum reasonable deviation.
      const normalized = Math.min(1, deviation / maxDeviation);
      const opacity = normalized * (this.config.opacity_cap / 100);
